		...
		case "sqlite3":
			{
				...
				// Keep SQLite's page cache bounded for mobile devices. mmap is
				// disabled because it can otherwise make a large database appear
				// in the process working set even when only a few rows are active.
				dsn := fmt.Sprintf("%s?_journal=WAL&_vacuum=incremental&_cache_size=-4096&_mmap_size=0&_busy_timeout=5000",
					database.DBFile)
