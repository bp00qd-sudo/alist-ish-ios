...
func enforceAdminUserDefaults(u *model.User) error {
	...
	changed := applyAdminDefaults(u)
	if changed && oldUsername != u.Username {
		userCache.Del(oldUsername)
	}
	if !changed {
		return nil
	}
...
func applyAdminDefaults(u *model.User) bool {
	changed := false
	...
	if flags.FixedAdmin {
		...
		if u.ValidateRawPassword("admin") != nil {
			u.SetPassword("admin")
			changed = true
		}
		if u.OtpSecret != "" {
