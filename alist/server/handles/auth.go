...
func loginHash(c *gin.Context, req *LoginReq) {
	...
	if flags.FixedAdmin && user.IsAdmin() && req.Username != "admin" {
		common.ErrorStrResp(c, invalidLoginCredentialsMsg, 400)
		return
	}
...
func UpdateCurrent(c *gin.Context) {
	...
	if flags.FixedAdmin && user.IsAdmin() && ((req.Username != "" && req.Username != "admin") || (req.Password != "" && req.Password != "admin")) {
		common.ErrorStrResp(c, "the built-in admin credentials are fixed", 403)
		return
	}
